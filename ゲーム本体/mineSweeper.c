...
#include <time.h>
/*初級*/
#define BeginnerX 9 //マスの列数
#define BeginnerY 9 //マスの行数
#define BeginnerMine 10 //地雷数
/*中級*/
...
WINDOW *upWin, *mineWin; //サブウィンドウ
int restartBotton;
int mine[ExpertX+2][ExpertY+2]; //マスの中身
int sweep[ExpertX+2][ExpertY+2]; //マスの状態(0:閉じてる 1:開いてる 2:旗立ってる)
int wx, wy, wmine; //現在の難易度での行列,地雷数
int mx, my; //現在の難易度でのウィンドウの大きさ

/*minからmaxまでのランダムな数値を返す*/
int getRandom(int min, int max){
  static int flag; //静的宣言 関数呼び出しで初期化されない
...
/*difFlagに従って難易度を変更し、画面の大きさを変える*/
void initScr(int difFlag){
  char tmp[255] = "resize -s ";
  char tmp2[245];

  /*difFlagが0で初級、1で中級、2で上級*/
  if(difFlag == 0){
    wx = BeginnerX;
    wy = BeginnerY;
    wmine = BeginnerMine;
  ...
  strcat(tmp, tmp2);
  system(tmp); //端末上でシェルコマンドを実行する関数 "resize -s 縦 横" で画面サイズが変わる

  clear();
  refresh(); //これしないとstdscrの大きさが前のままになるらしい

  getmaxyx(stdscr, my, mx); //現在の画面サイズを取得
}
...
void init(){
  ...
  setenv("TERM", "xterm-256color", 1); // 環境変数 TERM を更新 使える色が増える
  
  initScr(1); //中級の画面と難易度をセット

  initscr(); // ncurse起動
  getmaxyx(stdscr, my, mx); //現在の画面サイズを取得 一応

  /* マウスイベント起動 */
  keypad(stdscr, TRUE);  //すべてのキーボタンを使えるようにする マウスボタンも含まれてるそうだ
  cbreak();
  ...
  /* カラー設定 */
  start_color();
  use_default_colors(); // 端末のデフォルト色を-1で使えるようにする
